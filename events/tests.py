...
class EventApiTests(APITestCase):
    def setUp(self):
        ...
        self.register_url = reverse(
            "events:events-toggle-register", args=[self.event.id]
        )
    ...
    def test_list_contains_annotated_fields(self):
        ...
        self.assertEqual(
            response.data[0]["organizer"],
            self.organizer.full_name
        )
    ...
    def test_organizer_cannot_register_for_own_event(self):
        ...
        self.assertEqual(
            response.data["detail"], "You cannot register for your own event."
        )
    ...
    def test_successful_registration_and_email(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(
            self.event.attendees.filter(id=self.participant.id).exists()
        )
